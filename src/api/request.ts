...
export default function setupRequestsRoutes(app: any): void {
  ...
  app.get(
    ...
    async (req: CustomRequest, res: Response, next: NextFunction) => {
      try {
        const { data } = await service.GetClientRequests(req.user);
        return res.json(data);
      } catch (error) {
        next(error);
      }
  ...
  app.get(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        const { id } = req.params;
        const { data } = await service.GetClientRequestById(+id);
        return res.json(data);
      } catch (error) {
        next(error);
      }
  ...
  app.post(
    ...
    async (req: CustomRequest, res: Response, next: NextFunction) => {
      try {
        ...
        const { reqType, idRequest, comments, isClosed } = req.body;

        if (reqType === "employee") {
          const { data } = await service.CommentEmployeeRequest(
            idRequest,
            comments,
            isClosed,
            userId
          ...
          const { data } = await service.CommentClientRequest(
            idRequest,
            comments,
            isClosed,
            userId
